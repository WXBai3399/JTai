...
function swiperAnimateCache() {
    for (allBoxes = window.document.documentElement.querySelectorAll(".ani"), i = 0; i < allBoxes.length; i++) {
        ...
        //在此处开始给元素赋值和设置属性
        var slide_in = allBoxes[i].attributes["data-slide-in"],
            slide_out = allBoxes[i].attributes["data-slide-out"];
        if (slide_in) {
...
function swiperAnimate(a) {
    ...
    if (!isNullObj(tempOutAnimateStack)) {
        ...
        for (var i in tempOutAnimateStack) {
            tureOutAnimateStack.push({
                time: parseInt(i) - disparity,
                element: tempOutAnimateStack[i]
            });
